...
class LightChangeColorCommand(CommandInterface):
    """Comando concreto"""
...
class RemoteController:
    ...
    def __init__(self) -> None:
        self._buttons: Dict[str, CommandInterface] = {}
        self._undos: List[Tuple[str, str]] = []

    def button_add_command(self, key: str, command: CommandInterface) -> None:
        self._buttons[key] = command

    def button_pressed(self, key: str) -> None:
        if key in self._buttons:
            self._buttons[key].execute()
            self._undos.append((key, "execute"))

    def button_undo(self, key: str) -> None:
        if key in self._buttons:
            self._buttons[key].undo()
            self._undos.append((key, "undo"))
